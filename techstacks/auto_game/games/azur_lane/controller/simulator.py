...
class AzurLaneWindow(Window):
    def __init__(self, **kwargs):
        ...
        self.scene_prev = self.scene_cur = None
    ...
    def compare_with_pixel(self, pixels, threshold=1) -> bool:
        correct, wrong = 0, 0
        for x, y, rgb_int in pixels:
            if (pixel := self.get_pixel(x, y, as_int=True)) != rgb_int:
                if self.debug:
                    print(
                        f"WRONG: {x, y}, RGB: {parse_rbg_int2tuple(rgb_int)}(Required) != {parse_rbg_int2tuple(pixel)}(Real)")
                wrong += 1
            else:
                correct += 1
