# quote from: https://zhuanlan.zhihu.com/p/56033037

import cv2
import numpy as np


def img2strimg(frame, chars=None, K=5):
    """
    ...
    """
    idx = 0
    chars = chars or tuple(list(range(10)))
    chars_num = len(chars)
    ...
    for rows in labels_picked:
        ...
        for cols in rows:
            if cols <= shadow_bound:
                ...
                cv2.putText(canvas, str(chars[idx % chars_num]), (x, y), cv2.FONT_HERSHEY_PLAIN, 0.75, 1)
                idx += 1
                if idx == chars_num:
                    idx = 0
...
if __name__ == '__main__':
    ...
    str_img = img2strimg(img, chars=("F", "a", "y", "e",))
    cv2.imwrite("", str_img)
